...
- **就地电码浮签化（owner 9/24 三轮：::after 内联显码撑动布局「跳一跳」）**：行尾显码由 `::after` 内联（内容进文档流，行宽变化引发换行回流）改为**卡内绝对定位浮签**——悬浮于条目上方、随联动 activate/clear 显隐，`pointer-events:none` 不挡悬停链，卡内坐标带滚动补偿与边界钳制；行为锁（悬停显码/离开隐藏，丢接线即红）＋样式契约（绝对定位、旧 ::after 方案不得回流）；实测悬停前后卡片高度/滚动高/行位置零变化。
- **三角色评测修复批（2026-09-24 三 subagent 评测，四批落地）**：
  - **月界收口（P1）**：demo 时间轴/刻度/播放/面板「下一变化」/报池排序与在效选择全链改真实毫秒序（Date 真月历），报文日号经「相对现在的真实月历定位」归一（短月不再差一天），「现在」来源以参数注入（`examples/src/timeline-core.ts` 纯函数内核＋月末边界单测：9/30 23:50Z 建窗 144 格不死锁、北京时显「10月1日」非「31日」）；`TafExpandAt` 的 day 采「自层锚月 1 日起的连续日序」，`addTafLayer`/`setTafLayerTime`/`createTafTimeControl` 新增 `calendarAnchor`、`TafLayerItem.monthAnchor` 逐报锚（层内归一——跨月混合报池各报正确，发布日号在生效前一月的报经 ±15 天窗口判据归前月）；`renderCard` 北京时经 `observeTimeOf` 真实 Date 换算（零新 API）、`renderTafCard` 新增可选 `monthAnchor`（缺锚保持 %31 折回＝残余近似仅显示位，注释明示）；展示带月位「M月D日」消除跨月歧义。
  - **键盘通道（P1）**：两卡统一契约——可聚焦元素 Enter/Space 开合解码气泡、Esc 关闭、`aria-expanded` 挂触发元素＋气泡经 `aria-describedby` 接入读屏；METAR 卡 `focusin` 联动等价（Tab 聚焦词即点亮＋电码浮签）；TAF 行头 title 改 `data-code` 浮签（focus 显段电码、不再双气泡）、行头 Enter/Space 开该行合并解码表（「行头代表段、Tab stop 减半」决策保留）。
  - **一致性批（P2 高共识）**：`TIER_COLORS`/`ConditionTier` 随 `@metweave/leaflet` 导出（图例/面板/圆点同表取色，demo 图例内联色退役）；TAF tooltip 摘要与「TEMPO 发作可能」置顶提示人话化（`summarizeTafConditions` 随 `@metweave/render` 导出，gloss 词表单一来源）；demo 内嵌 39 站中文站名（中文在前英文括注、全链生效）；METAR 卡移除原生 title（aria-label＋气泡＋浮签三通道，双气泡叠出收口）。
  - **工程债批（P2）**：切回实况停播；实况拉取 60s 超时；METAR 卡限高对齐 TAF 契约（`min(65vh, 680px)` 内滚）；`#mode-bar` role 改 toolbar；播放期间面板冻结行序（停播即恢复档位排序）；时间轴「现在」锚漂移重锚（越窗尾/漂移超 30 分钟重算窗、格位保持）；面板档位数据直读（`tafTierOf` 随 `@metweave/leaflet` 导出，不再从 marker DOM className 正则回读）；favicon 空数据 URI 声明（首页零 404）。
  - **小白体验批**：页面导览一句＋按钮 title 白话（实况/预报时点不同的解释）；时间轴「回到现在」按钮；北京时前缀去重（段区间尾端收敛）；METAR 卡 RAW 区身份行；图例项悬停判据（注明本库自拟）；免责声明去行话；上一周期整体缺失的可见降级提示；**变化可见性**——「现在」格为基准，变差站圆点加橙描边、面板「相对现在」列（`tierChangeOf`）。
- **两卡交互统一（owner 9/24 定稿：联动语言＝点亮＋浮签、**不做压暗**；点击＝解码气泡）**：TAF 卡移除联动压暗（mw-taf-dim 整体退役，契约锁不得回流）；METAR 卡补**电码浮签**——悬停主表词在点亮两侧的同时随行显出 RAW 侧组电码（复用其 data-hint 单一来源映射，行为锁）；TAF 卡补**点击解码气泡**——点击要素条目弹「电码→人话」逐组/逐层行（词表与分段行同一 gloss 单一真相）＋ FM 51 依据行（条款号核自 WMO-No. 306 卷 I.1 2019 版 FM 51–XV：风 §51.3 / 能见度 §51.4 / 天气 §51.5（NSW §51.5.1）/ 云 §51.6；变化组条款未核不标），开合语义与 METAR 卡一致（点已开收起/点空白收起）；两卡从此同契约——悬停＝映射＋显码、点击＝详解＋依据。
- **左上导览错开（owner 9/24「重叠了，需要错开」二轮裁定：高度复原、只横向错开）**：导览卡 left 10→52（缩放控件右缘+8，与 +/− 同高并排）、弹窗 autopan 左上避让按新几何重算 (12,84)→(426,110) 罩住控件+导览块；layout 锁四值契约（top=10/left≥52/避让 x·y），三突变验红；实测 rect 零相交、弹窗定格恰落角点。
- **站点面板系列修（owner 9/25 四轮反馈）**：① 站点列表按钮 hidden 初态回归（两模式常在改造误删唯一解除语句→永不可见）——HTML 去 hidden、title 双模式口径；② 面板表头随模式切换（实况 3 列表体套预报 4 列表头，实况摘要在「下一变化」名下）——`setPanelHead` 随模式换标+第四列隐藏；③「相对现在」列改名「**查看时刻 vs 现在**」（列名省略对比主语、与左邻列连读歧义）——表头写明对比双方+表头/单元格双人话 tooltip；④ 面板随模式自动切换（面板开着切实况/预报列表不更新；切回实况顺手关面板的旧逻辑一并放开）——`setMode` 接 `repanelOnMode` 延迟回调。
- **站点预报面板加宽（owner 9/25「太挤」→「不能有内部滚动」两轮）**：320→392 定值后仍有内部横滚（nowrap 撑出的最小宽超出面板）——改 `width: max-content`（内容最小宽即面板宽，全量展示）+ `max-width: min(560px, 100vw−40px)` 超屏兜底；自动表格布局（fixed 均分列宽实测 39 行全折已回退）：变化列与表头 nowrap 按内容取宽、站点列吸收剩余；实测 TAF 四列 25/290/144/101、METAR 三列 531，两模式零横向滚动。
- **产品级挂起启动批（owner 9/24 指令：站点列表/双日界引用/双源报池 三项落地）**：① METAR 模式站点列表——`@metweave/leaflet` 新增公共面 `metarTierOf`（档位判据与圆点同一管线）与 `summarizeMetarConditions`（要素摘要），demo 站点面板双模式化（实况＝档色点+站码+站名+实况摘要静态行、预报＝既有下一变化动态行，列表按钮两模式常在）；② UTC/北京时双日界引用——`@metweave/render` gloss 新增 `utcDayRefText`，TAF 卡发布/查看/有效期端点与 METAR 卡观测时刻行、demo 时间轴 `fmtTl` 在北京时制下跨 UTC 日时括注「（UTC M月D日）」（同日不括注；段头不加防噪；病态折回路径无 UTC 锚不加）；③ TAF 双源报池——`metweave/sources` 新增 ogimet 补充线（`ogimetTafUrl` display_metars2.php tipo=FT 配方＝私有管线同源 / `parseOgimetTafs` <pre> 区 12 位前缀起报+续行并单 / `getTafsOgimet` 单站近窗、空窗返回空数组的正常降级口径），demo 按「aviationweather 最新优先、ogimet 补最新/次新」合并（只补池不足两份的缺口站、6 路并发、10s 超时、失败静默、raw 去重天然合池），实测本时次 aviationweather 上一周期空窗、ogimet 补齐 38 站且总站点 38→39；examples 代理新增 /ogimet-taf（rewrite + referer 头）。三角色遗留面板交互语言两模式一致。
- **文档对账批（owner 9/24「缺报文档覆盖目前所有能力」指令）**：伞包/core README 补 TAF 双线（TafReport IR 族/三错误码/取数双线/换源位/stations-cn）、根 README 补「30 秒认识 TAF」与示例双模式能力清单、render README 补 summarizeTafConditions/DECODE_CITES、docs 索引收 error-codes.md 并更新双矩阵描述。
