...
TAF 侧夹具沿同一纪律组织，`source` 取值在上述之外新增：

- `ogimet`＝OGIMET 历史回填仓（本机数据区 raw/ogimet），溯源三件套 `station` + `obsTime` + `hash`（记录级哈希，可回查原行）；
- `教材`＝W39 补全清单/taf-tac 教材引用形态（条文示例，公开国际通路实测 0 出现的形态——如 NIL/CNL/时组后 COR，实证缺席本身已入档）。

批 1 覆盖电头三形态（裸/AMD/COR 类型词位）；NIL/CNL、基况段、变化组夹具随对应批次扩充。`trailing-equals` 雷点标注中国站传输层终止符 `=`（A1/1.2 处理前如实保留）。
